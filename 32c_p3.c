...
int main()
{
	...
	int shmid = shmget(key1, 200, 0);
	char *ptr = (char *)shmat(shmid, NULL, 0);

	int semid = semget(key2, 1, 0);

	printf("Program 3 will attempt to access Critical Section and go to sleep for 60 seconds\n");

	union semun arg;
	int semval = semctl(semid, 0, GETVAL, arg);
	if(semval == 0)
	{
		printf("Semaphore value is 0, so process 3 is blocked from entering\n");
		while((semctl(semid, 0, GETVAL, arg)) == 0);
	}
	printf("Semaphore value is greater than 0 now, so process 3 is allowed to enter\n");
	...
	printf("Process 3 entering CS after reducing sem value\n");
	sleep(60);
	buf.sem_op = 1;
