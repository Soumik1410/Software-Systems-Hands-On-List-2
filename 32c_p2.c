...
int main()
{
	...
	printf("Process 2 will attempt to access Critical Section and go to sleep for 60 seconds\n");

	union semun arg;
	int semval = semctl(semid, 0, GETVAL, arg);
	if(semval == 0)
	{
		printf("Semaphore value is 0, so blocked from entering\n");
		while((semctl(semid, 0, GETVAL, arg)) == 0);
	}
	printf("Semaphore value is greater than 0 now, so program 2 is allowed to enter\n");
	...
	printf("Process 2 entering CS after reducing sem value\n");
	sleep(60);
	buf.sem_op = 1;
